...
def main():
    ...
    print("\n\n")
    print("=" * 20 + " LR Peak (Only if different) " + "=" * 20)
    count = 0
    lr_printed_count = 0
    for filepath in files_to_process:
        count += 1
        ...
        if filepath in results_data:
            ...
            if result_info['main_error']:
                 # 使用 :03d 格式化编号
                 print(f"{count:03d}. {filename_without_ext}:")
            ...
            should_print_lr = True
            if not left_err and not right_err:
                if (left_val, left_unit) == (right_val, right_unit):
                    should_print_lr = False

            if should_print_lr:
                lr_printed_count += 1
                # 使用 :03d 格式化编号
                print(f"{count:03d}. {filename_without_ext}:")
                if not left_err: print(f"   Left: {left_val} {left_unit}")
                else: print(f"   Left: 检测失败 ({left_err})")
                if not right_err: print(f"   Right: {right_val} {right_unit}")
    ...
    print(f"  - LR Peak 部分显示了 {lr_printed_count} 个条目 (左右声道不同或检测失败)。")
